...
class CreateSession(commands.Cog):
    ...
    async def createsession(self, ctx, member: Option(
        ...
        category = discord.utils.get(ctx.guild.categories,
                                     id=986411485486006302)
        vc = await ctx.guild.create_voice_channel(
            category=category,
            name=f"{ctx.author.name}'s Voice Channel",
            overwrites={
                ctx.author:
        ...
        await ctx.respond(embed=discord.Embed(
            title="Success!",
            description=
            f"Successfully created a private session between {ctx.author.mention} and {member.mention}\nIt can be found at - <#{tc.id}>",
            color=discord.Color.green()))

        await tc.send(
            f"{ctx.author.mention}\n{member.mention}",
            embed=discord.Embed(
                title="Have a fun class <3",
                description=
                f"I hope you have a fun session at <#{vc.id}>, also, when you're done, press the button below to end the session.",
                color=discord.Color.green()),
            view=MyView(ctx, tc, self.client, member))
