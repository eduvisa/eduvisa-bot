...
class Report(commands.Cog):
  ...
  @slash_command(name="report", description="Report a bug, get EdCoins as a reward hehe!")
  async def report(self,ctx,bug:Option(str,"What the bug is",required=True)):
    await ctx.respond(embed=discord.Embed(title="Successfully reported the bug <a:blobdance:853873081571344394>",description="Our developers (okay there's only one, **Achintya#1111**) will get back to you soon!",color=discord.Color.nitro_pink()))
    channel = await ctx.guild.fetch_channel(channelId)
    await channel.send(embed=discord.Embed(title=f"{ctx.author} reported a bug",description=f"{bug}",color=discord.Color.dark_red()))
